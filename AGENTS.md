...
- For packages with one major in the tree, the override is a same-major bump (`flatted`, `picomatch`, `yaml`, `fast-uri`).
- For packages with multiple coexisting majors (`minimatch` 3/8/9, `ajv` 6/8, `brace-expansion` 1/2), the override **collapses them onto the newest safe major** (`minimatch@^9.0.7`, `ajv@^8.18.0`, `brace-expansion@^2.0.3`). This was verified safe empirically: `bun run lint` (oxlint + `eslint-plugin-obsidianmd` JS plugin), `bun run build`, and direct ESLint config-schema compilation (the `@eslint/eslintrc` + `ajv` codepath) all pass. These are dev-only lint-time deps, so the collapse has no effect on the shipped plugin.
...
- When adding any dependency, re-run `bun audit`. If new transitive findings appear, extend `overrides` with the minimum safe version and confirm `bun run review` stays green.
